...
// Драйвер обробки: 'gd' або 'imagick'; фолбек на проектний IMAGE_DRIVER
...
### Перевірка кешу пресетів

```bash
php artisan imagepresets:verify
```

Знаходить пошкоджені закешовані файли пресетів (обірваний encode/запис) та осиротілі `*.tmp*`-залишки процесів, вбитих до атомарного rename. За замовчуванням — dry run, без прапорця нічого не видаляється.
...
| `--disk=` | Перевизначити диск (за замовчуванням: `imagepresets.disk`) |
| `--path=` | Перевизначити шлях (за замовчуванням: `imagepresets.path`) |
| `--delete` | Видалити знайдені пошкоджені/осиротілі файли |
| `--deep` | Додатково декодувати кожен webp і позначати файли з пошкодженим payload (suspects) |
| `--gray-threshold=25` | Відсоток семплів сірої заливки, вище якого webp вважається підозрілим (з `--deep`) |
| `--delete-suspects` | Видалити підозрілі webp, знайдені `--deep` |

Структурна перевірка ловить файли, чий розмір не збігається з внутрішнім обліком формату (обірваний запис). `--deep` націлений на підступніший випадок: довжина файлу правильна, але частина VP8-payload так і не потрапила на диск — libwebp тоді декодує зображення, заливаючи решту рядків суцільним сірим `rgb(128,128,128)`. Такі файли проходять структурну перевірку і ловляться лише декодуванням.

Suspects — це евристика: зображення зі справді сірими ділянками може бути хибним спрацюванням. Перегляньте список перед видаленням; `--delete` підозрілі файли не чіпає — лише явний `--delete-suspects`. Видалені файли просто перегенеруються при наступному запиті.

```bash
# Показати все, включно з підозрілими за decode-перевіркою
php artisan imagepresets:verify --deep

# Видалити пошкоджені/осиротілі, а після перегляду — і підозрілі
php artisan imagepresets:verify --delete
php artisan imagepresets:verify --delete-suspects
```

Щоб відхиляти пошкоджений webp ще на етапі генерації — до того, як він потрапить у кеш і в довгоживучі immutable HTTP-кеші за ним — увімкніть opt-in конфіг:

```dotenv
IMAGEPRESET_VERIFY_DECODE=true
```

Відхилена генерація повертає 404 і повторюється при наступному запиті.
