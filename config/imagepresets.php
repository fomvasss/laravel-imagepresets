...
return [
    ...
    */
    ...
    | Trusted Bypass
    |--------------------------------------------------------------------------
    | When enabled, URLs generated by imagepreset_url(), Imagepreset::url(),
    | and @imagepreset() with $bypass=true include a server-signed token (_t).
    | A valid token allows the request to skip allowed_widths / allowed_heights /
    | allowed_sizes / allowed_qualities / allowed_fits / allowed_formats checks.
    |
    | Security: the token is HMAC-SHA256 signed with APP_KEY and cannot be
    | forged without the key. Path traversal, remote host, and max pixel checks
    | are always enforced regardless of the token.
    |
    | Enable only on sites that need backend-driven dynamic sizes (e.g. Blade).
    | Leave disabled (default) on public API endpoints.
    */
    'trusted_bypass' => env('IMAGEPRESET_TRUSTED_BYPASS', false),
